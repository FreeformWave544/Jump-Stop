...
func _on_body_entered(body: Node2D) -> void:
	...
	if body is Player:
		...
		if is_inside_tree(): get_tree().change_scene_to_packed(next_level if (next_level and ResourceLoader.exists(next_level.resource_path)) else "res://Scenes/main_menu.tscn")
